  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 80; i++) {
      particles.push({
        ...
        size: 1.5 + Math.random() * 4, baseOp: 0.35 + Math.random() * 0.45,
        vx: (Math.random() - 0.5) * 0.15, vy: -(0.06 + Math.random() * 0.14),
    ...
    const loop = () => {
      ...
      particles.forEach((p) => {
        ...
        const op = p.baseOp * (0.5 + 0.5 * Math.abs(Math.sin(t * p.twinkleSpeed + p.twinkleOffset)));
        ctx.save(); ctx.globalAlpha = op; ctx.fillStyle = "rgb(196,168,130)";
        if (p.type === "star") { drawStar(p.x, p.y, p.size); ctx.fill(); }
  ...
  return (
    <main className="relative min-h-screen bg-canvas overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-8 py-6 flex items-center justify-between">
        <span className="font-serif text-base tracking-[0.22em] uppercase text-noir">Zesira Studio</span>
      ...
      <section className="relative min-h-screen flex items-center justify-center px-8 z-10">
        <div className="text-center max-w-4xl mx-auto">
          <p className="font-sans text-[10px] tracking-[0.26em] uppercase text-sand mb-10 opacity-0 animate-fadeUp" style={{ animationDelay: "0.3s", animationFillMode: "forwards" }}>Digital Atelier</p>
          <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl text-noir leading-[1.25] italic mb-8 opacity-0 animate-fadeUp" style={{ animationDelay: "0.6s", animationFillMode: "forwards" }}>
            Some brands try to be noticed.<br />Others simply leave a mark.
          </h1>
      ...
      <section id="work" className="relative z-10 py-32 px-8 md:px-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="py-24 text-center border-t border-b border-sand/15">
            ...
            <a href="#contact" className="inline-block font-sans text-[10px] tracking-[0.22em] uppercase text-noir border-b border-sand/50 pb-1 hover:text-sand transition-colors duration-300">
      ...
      <section id="services" className="relative z-10 py-32 px-8 md:px-16">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-0">
            {[
              { num: "I", name: "Starter Presence", feeling: "Your first step into the world — quiet, but unforgettable.", transformation: "From invisible to felt. A minimal but emotionally resonant digital home that speaks before you do.", for: "New brands, solo founders, first digital presence.", cta: "Begin quietly →" },
              { num: "II", name: "Editorial Brand Site", feeling: "Your brand, given a cinematic world to live in.", transformation: "From a website to an experience. Every scroll reveals something new. Every section feels intentional.", for: "Established brands ready to go deeper.", cta: "Build your world →" },
              { num: "III", name: "Luxury Launch Experience", feeling: "Not a launch. An arrival.", transformation: "A complete digital universe — site, motion, visuals, creative direction, launch strategy. Everything aligned. Everything felt.", for: "Brands that deserve more than a website.", cta: "Let's talk →" },
            ].map((pkg) => (
              <div key={pkg.num} className="group border-t border-sand/20 py-16 grid md:grid-cols-[80px_1fr_1fr] gap-8 hover:bg-linen/30 transition-colors duration-700 px-4">
      ...
      <section id="contact" className="relative z-10 py-32 px-8 md:px-16">
        <div className="max-w-2xl mx-auto">
          <p className="font-sans text-[10px] tracking-[0.22em] uppercase text-sand mb-8 text-center">Contact</p>
          <h2 className="font-serif italic text-noir text-3xl md:text-5xl leading-[1.3] mb-6 text-center">
            Tell us about<br />your world.
          </h2>
          <p className="font-serif italic text-noir/50 text-sm text-center mb-16 leading-loose">
          ...
          {sent ? (
            ...
            <div className="space-y-10">
              <div className="border-b-2 border-sand/50 pb-2">
                <label className="font-sans text-[11px] tracking-[0.18em] uppercase text-noir/70 block mb-3">Your name</label>
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="w-full bg-transparent font-serif italic text-noir text-lg outline-none placeholder:text-noir/25"
                  placeholder="How should we address you?" />
              </div>
              <div className="border-b-2 border-sand/50 pb-2">
                <label className="font-sans text-[11px] tracking-[0.18em] uppercase text-noir/70 block mb-3">Your brand</label>
                <input type="text" value={formData.brand} onChange={(e) => setFormData({ ...formData, brand: e.target.value })}
                  className="w-full bg-transparent font-serif italic text-noir text-lg outline-none placeholder:text-noir/25"
                  placeholder="What world are we building?" />
              </div>
              <div className="border-b-2 border-sand/50 pb-2">
                <label className="font-sans text-[11px] tracking-[0.18em] uppercase text-noir/70 block mb-3">Your message</label>
                <textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  rows={4} className="w-full bg-transparent font-serif italic text-noir text-lg outline-none placeholder:text-noir/25 resize-none"
      ...
      <style jsx global>{`
        ...
        @keyframes drift1 { from { transform: translate(0,0); } to { transform: translate(-18px, 22px); } }
        @keyframes drift2 { from { transform: translate(0,0); } to { transform: translate(16px, -18px); } }
