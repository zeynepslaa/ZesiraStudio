    ...
    <main className="relative overflow-hidden">
      ...
      <section className="relative min-h-[100svh] overflow-hidden px-4 pb-12 pt-28 md:pt-36">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 h-44 bg-gradient-to-t from-charcoal/80 to-transparent" />
        </motion.div>

        <div className="section-shell grid min-h-[calc(100svh-9rem)] items-center gap-10 lg:-mt-12 lg:grid-cols-[1.05fr_0.95fr]">
          <motion.div {...fadeUp} className="max-w-4xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-champagne/25 bg-porcelain/70 px-4 py-2 text-xs uppercase tracking-[0.28em] text-champagne/85 backdrop-blur">
              <Sparkles size={14} />
              Not just CC. A living universe.
            </div>
            <h1 className="text-balance font-display text-6xl font-semibold leading-[0.85] text-espresso sm:text-7xl md:text-8xl xl:text-[8.5rem]">
              Zesira Studio
            </h1>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a
                href="#world"
                className="group inline-flex items-center justify-center gap-3 rounded-full bg-espresso px-6 py-4 text-sm font-semibold text-porcelain transition hover:bg-champagne"
              >
                Enter The World
              ...
              <a
                href="#downloads"
                className="inline-flex items-center justify-center gap-3 rounded-full border border-espresso/14 bg-porcelain/70 px-6 py-4 text-sm font-semibold text-espresso backdrop-blur transition hover:border-champagne/45 hover:bg-porcelain/10"
              >
                Download Collection
                <Download size={16} />
              </a>
            ...
            <div className="glass-line absolute bottom-0 right-6 w-[62%] rounded-[1.75rem] p-5 backdrop-blur-xl">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 font-display text-3xl leading-none text-espresso">
                    12 households active
                  </p>
                </div>
                <UsersRound className="text-champagne" />
              </div>
              <div className="mt-5 h-1.5 overflow-hidden rounded-full bg-cream/55">
                <motion.div
                  initial={{ width: "0%" }}
                  animate={{ width: "72%" }}
          ...
          <div className="mt-14 grid gap-5 lg:grid-cols-3">
            {districts.map((district, index) => (
              <motion.article
                ...
                className="group glass-line rounded-[1.75rem] p-5 transition duration-500 hover:-translate-y-1 hover:border-champagne/30 hover:bg-cream/55"
              >
                <EditorialVisual
                  label={district.name}
                  tone={district.mood}
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {district.businesses.map((business) => (
                    <span
                      key={business}
                      className="rounded-full border border-espresso/10 px-3 py-1.5 text-xs text-fog"
                    >
                      {business}
                    </span>
                  ))}
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {characters.map((character, index) => (
              <motion.article
                ...
                className="group relative overflow-hidden rounded-[1.75rem] border border-espresso/10 bg-porcelain/70 p-4 shadow-editorial"
              >
                ...
                <div className="relative">
                  ...
                  <div className="pt-5">
                    ...
                    <p className="mt-4 rounded-2xl border border-champagne/18 bg-cream/60 p-3 text-sm italic leading-6 text-espresso">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {character.traits.map((trait) => (
                        <span
                          key={trait}
                          className="rounded-full bg-cream/70 px-3 py-1 text-xs text-espresso"
                        >
                          {trait}
                        </span>
                      ))}
          ...
          <motion.div {...fadeUp}>
            ...
            <div className="mt-8 grid gap-3">
              {["Luna Supper Club", "Vale Atelier", "Sable Pier Rental"].map((build) => (
                <div
                  key={build}
                  className="flex items-center justify-between rounded-2xl border border-espresso/10 bg-porcelain/70 p-4"
                >
                  <span className="font-display text-2xl text-espresso">{build}</span>
            ...
            <div className="glass-line rounded-[1.75rem] p-6 sm:col-span-2">
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <h3 className="mt-2 font-display text-4xl text-espresso">
                    Luna Supper Club
                  </h3>
                </div>
                <div className="flex gap-3 text-sm text-fog">
                  <span className="rounded-full border border-espresso/10 px-4 py-2">
                    30x20
                  </span>
                  <span className="rounded-full border border-espresso/10 px-4 py-2">
                    Restaurant
      ...
      <section id="timeline" className="py-24 md:py-32">
        <div className="section-shell">
          ...
          <div className="relative mx-auto mt-16 max-w-4xl">
            <div className="absolute left-4 top-2 h-[calc(100%-1rem)] w-px bg-gradient-to-b from-champagne via-espresso/15 to-transparent md:left-1/2" />
            {timeline.map((event, index) => (
              ...
              >
                <div className="ml-10 rounded-[1.5rem] border border-espresso/10 bg-porcelain/70 p-5 shadow-glow backdrop-blur md:ml-0">
                  <div className="flex items-center gap-3 text-champagne">
                    <CalendarDays size={17} />
                    <span className="text-xs uppercase tracking-[0.26em]">{event.date}</span>
                  </div>
                  <h3 className="mt-4 font-display text-3xl font-semibold text-espresso">
                    {event.title}
                  </h3>
                  <p className="mt-3 text-sm leading-7 text-fog">{event.body}</p>
                  <div className="mt-5 flex flex-wrap gap-2">
                    {event.tags.map((tag) => (
                      <span key={tag} className="rounded-full bg-cream/55 px-3 py-1 text-xs text-fog">
                        {tag}
                      </span>
                    ))}
                  </div>
                </div>
                <div className="absolute left-2.5 top-6 h-3.5 w-3.5 rounded-full border border-champagne bg-porcelain md:left-[calc(50%-7px)]" />
              </motion.article>
            ))}
          </div>
        </div>
      ...
      <section id="downloads" className="py-24 md:py-32">
        <div className="section-shell">
          <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
            ...
            />
            <div className="flex rounded-full border border-espresso/10 bg-porcelain/70 p-1 text-sm text-fog">
              {["All", "Sims", "Builds", "Save"].map((tab) => (
                <button
                  key={tab}
                  type="button"
                  className="rounded-full px-4 py-2 transition first:bg-espresso first:text-porcelain hover:text-espresso"
                >
                  {tab}
                </button>
              ))}
          ...
          <div className="mt-12 grid gap-4 lg:grid-cols-4">
            {downloads.map((item, index) => (
              <motion.article
                ...
                {...fadeUp}
                transition={{ ...fadeUp.transition, delay: index * 0.06 }}
                className="rounded-[1.5rem] border border-espresso/10 bg-porcelain/70 p-5 transition hover:-translate-y-1 hover:border-champagne/30 hover:bg-cream/55"
              >
                ...
                <h3 className="mt-6 font-display text-3xl font-semibold text-espresso">
                  {item.title}
                </h3>
                <p className="mt-4 text-sm leading-7 text-fog">{item.detail}</p>
                <div className="mt-6 flex items-center justify-between border-t border-espresso/10 pt-4 text-xs uppercase tracking-[0.18em] text-fog/80">
                  <span>{item.tier}</span>
      ...
      <section id="patreon" className="py-24 md:py-32">
        <div className="section-shell overflow-hidden rounded-[2rem] border border-champagne/20 bg-porcelain/75 p-6 shadow-editorial md:p-10">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              ...
              <h2 className="font-display text-5xl font-semibold leading-[0.95] text-espresso md:text-7xl">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-8 inline-flex items-center gap-3 rounded-full bg-espresso px-6 py-4 text-sm font-semibold text-porcelain transition hover:bg-champagne"
              >
                Join Patreon
                <Heart size={16} />
              </a>
            ...
            <div className="grid gap-4">
              {patreonTiers.map(({ tier, detail, Icon }) => (
                <div
                  key={tier}
                  className="rounded-[1.25rem] border border-espresso/10 bg-porcelain/70 p-5"
                >
                  <div className="flex items-center justify-between">
                    <h3 className="font-display text-3xl text-espresso">{tier}</h3>
        ...
        <div className="section-shell">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {diaryPosts.map((post) => (
              <article
                key={post.title}
                className="rounded-[1.5rem] border border-espresso/10 bg-porcelain/70 p-6 transition hover:border-champagne/30 hover:bg-cream/55"
              >
                <p className="text-xs uppercase tracking-[0.26em] text-champagne/75">
                  {post.category}
                </p>
                <h3 className="mt-5 font-display text-3xl font-semibold leading-none text-espresso">
                  {post.title}
                </h3>
      ...
      <footer className="border-t border-espresso/10 py-10">
        <div className="section-shell flex flex-col gap-4 text-sm text-fog/75 md:flex-row md:items-center md:justify-between">
          <p className="font-display text-2xl text-espresso">Zesira Studio</p>
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 self-start rounded-full border border-espresso/10 px-4 py-2 text-xs uppercase tracking-[0.2em] text-fog transition hover:text-espresso md:self-auto"
          >
            <Music2 size={14} />
            Sound Off
          </button>
